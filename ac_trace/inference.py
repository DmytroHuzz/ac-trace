...
from ac_trace.python_ast import (
    PythonSymbol,
...
def _covered_lines_for_selector(
    project_root: Path, source_paths: list[str], selector: str
) -> dict[Path, set[int]]:
...
def _group_code_refs(code_refs: list[CodeRef]) -> list[CodeRef]:
    ...
    for code_ref in sorted(
        code_refs, key=lambda ref: (ref.path, ref.symbol or "", ref.lines or "")
    ):
...
def _filter_specific_symbols(symbols: list[PythonSymbol]) -> list[PythonSymbol]:
    ...
    for candidate in sorted(
        symbols, key=lambda symbol: (symbol.lineno, symbol.end_lineno, symbol.qualname)
    ):
        is_container = any(
...
def infer_manifest(catalog: CriteriaCatalog) -> TraceManifest:
    ...
    criteria_by_id = {
        criterion.id: criterion for criterion in catalog.acceptance_criteria
    }
    tests_by_criterion: dict[str, list[tuple[str, str, str]]] = {
        criterion.id: [] for criterion in catalog.acceptance_criteria
    }
    for test_case in discovered_tests:
        ...
        if unknown_ids:
            ...
            raise InferenceError(
                f"{test_case.selector} references unknown acceptance criteria: {unknown_text}"
            )
        for criterion_id in test_case.ac_ids:
            tests_by_criterion[criterion_id].append(
                (test_case.path, test_case.case_id, test_case.selector)
            )
    ...
    for criterion in catalog.acceptance_criteria:
        ...
        for _, _, selector in mapped_tests:
            coverage = _covered_lines_for_selector(
                catalog.project_root, catalog.source_paths, selector
            )
            for covered_path, executed_lines in coverage.items():
                try:
                    relative_path = covered_path.relative_to(
                ...
                for symbol in discover_python_symbols(covered_path):
                    if any(
                        symbol.body_lineno <= line <= symbol.end_lineno
        ...
        if not inferred_code:
            raise InferenceError(
                f"{criterion.id} did not map to any Python symbols under {catalog.source_paths}"
            )
        ...
        acceptance_criteria.append(
            AcceptanceCriterion(
                ...
                tests=_group_tests_by_path(
                    [(path, case_id) for path, case_id, _ in mapped_tests]
                ),
            )
        )
